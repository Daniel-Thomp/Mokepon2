...
public class battle {
    ...
    public static void startBattle(){
        ...
        Timer timer = new Timer(2500, event -> {
            ...
            platform1.setBounds(300,195,372,100);
            ...
            executor.scheduleAtFixedRate(animations.openingAnimation, 0, (int)((1.0/360.0)*1500.0), TimeUnit.MILLISECONDS);
        });
    ...
    public static void calcDamage(int power, int type, String user, String category, String effect, int effectStrength, String effect2, int effectStrength2, String moveName, int accuracy){
            ...
            Timer timer = new Timer(delay, event -> {
                ...
                if (ThreadLocalRandom.current().nextInt(1, 101) <= accuracy || accuracy == 0) {
                    if (power != 0) {
                        ...
                        damage = ((((2*(double)attacker.level)/5+2)*(double)power*((double)attack/(double)defence))/50)+2;
    ...
    public static void effect(String effect, int effectStrength, Mokepon user, Mokepon opponent){
        ...
        if (effect == null) {
            ...
            } else if ((effectStrength == 0) && (effect == "Para" || effect == "Freeze" || effect == "Burn" || effect == "Sleep" || effect == "Poison" || effect == "BadlyPoison")) {
                Main.animator.text("But it failed");
            }
            if (effect == "Confuse") {
                //do a confusion
                ...
                if ((user == Main.Player.inBattle && move.playerFirst)||(user == Main.Opponent.inBattle && move.playerFirst == false)) {
    ...
    public static void drawHealthbar(){
        ...
        int healthPercentO = (int)(((double)Main.Opponent.inBattle.currentHP/(double)Main.Opponent.inBattle.maxHP)*100);             
        ...
        int healthPercentP = (int)(((double)Main.Player.inBattle.currentHP/(double)Main.Player.inBattle.maxHP)*100);             
    ...
    public static void endBattle(){
        if (Main.Player.inBattle.xp > (int)(Math.pow(Main.Player.inBattle.level, 3)-Math.pow(Main.Player.inBattle.level-1.0, 3))) {
            Main.Player.inBattle.xp = Main.Player.inBattle.xp-(int)(Math.pow(Main.Player.inBattle.level, 3)-Math.pow(Main.Player.inBattle.level-1.0, 3));
            ...
            screen.frame.remove(cursorLabel);
